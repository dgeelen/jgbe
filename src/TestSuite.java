public class TestSuite
{
    ...
    public int diagnose( boolean verbose ) {
      ...
      result = addrr8b_diag();
      if ( verbose && result ) {
        System.out.println( "ADDrr8b instruction appears to work ok" );
        }
      else {
        System.out.println( "*ERROR* IN ADDrr8b INSTRUCTION!" );
        ++count;
        }
    ...
    private boolean addrr8b_diag() {
      ...
      */
      ...
      cpu.regs[cpu.FLAG_REG] = 0; // clear all flags
      cpu.addrr8b( cpu.A, cpu.B );
      if ( cpu.regs[cpu.A]!= 0x0f ) {
        System.out.println( "Error: ADDrr8b: 0x07 + 0x08 != 0x0f" );
        status = status && false;
        }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.ZF_Mask) == cpu.ZF_Mask) {
        System.out.println( "Error: ADDrr8b: 0x07 + 0x08 = 0x0f and ZF is set" );
        status = status && false;
      }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.NF_Mask) != cpu.NF_Mask) {
        System.out.println( "Error: ADDrr8b: does not set NF" );
        status = status && false;
      }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.HC_Mask) == cpu.HC_Mask) {
        System.out.println( "Error: ADDrr8b: 0x07 + 0x08 = 0x0f and HC is set" );
        status = status && false;
      }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.CF_Mask) == cpu.CF_Mask) {
        System.out.println( "Error: ADDrr8b: 0x07 + 0x08 = 0x0f and CF is set" );
        status = status && false;
      }
      cpu.regs[cpu.A]=0x07; cpu.regs[cpu.B]=0x08;
      cpu.regs[cpu.FLAG_REG] = 0xf0; // set all flags
      ...
      cpu.regs[cpu.A]=0x0f; cpu.regs[cpu.B]=0x01;
      cpu.regs[cpu.FLAG_REG] = 0; // clear all flags
      cpu.addrr8b( cpu.A, cpu.B );
      if ( cpu.regs[cpu.A]!= 0x10 ) {
        System.out.println( "Error: ADDrr8b: 0x0f + 0x01 != 0x10" );
        status = status && false;
        }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.ZF_Mask) == cpu.ZF_Mask) {
        System.out.println( "Error: ADDrr8b: 0x0f + 0x01 = 0x10 and ZF is set" );
        status = status && false;
      }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.NF_Mask) != cpu.NF_Mask) {
        System.out.println( "Error: ADDrr8b: does not set NF" );
      ...
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.HC_Mask) != cpu.HC_Mask) {
        System.out.println( "Error: ADDrr8b: 0x0f + 0x01 = 0x10 and HC is NOT set" );
        status = status && false;
      }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.CF_Mask) == cpu.CF_Mask) {
        System.out.println( "Error: ADDrr8b: 0x0f + 0x01 = 0x10 and CF is set" );
        status = status && false;
      }
      cpu.regs[cpu.A]=0x0f; cpu.regs[cpu.B]=0x01;
      cpu.regs[cpu.FLAG_REG] = 0xf0; // set all flags
      ...
      cpu.regs[cpu.A]=0x10; cpu.regs[cpu.B]=0xf0;
      cpu.regs[cpu.FLAG_REG] = 0; // clear all flags
      cpu.addrr8b( cpu.A, cpu.B );
      if ( cpu.regs[cpu.A]!= 0x00 ) {
        System.out.println( "Error: ADDrr8b: 0x10 + 0xf0 != 0x00" );
        status = status && false;
        }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.ZF_Mask) != cpu.ZF_Mask) {
        System.out.println( "Error: ADDrr8b: 0x10 + 0xf0 = 0x00 and ZF is NOT set" );
        status = status && false;
      }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.NF_Mask) != cpu.NF_Mask) {
        System.out.println( "Error: ADDrr8b: does not set NF" );
      ...
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.HC_Mask) == cpu.HC_Mask) {
        System.out.println( "Error: ADDrr8b: 0x10 + 0xf0 = 0x00 and HC is set" );
        status = status && false;
      }
      if ( (cpu.regs[cpu.FLAG_REG]&cpu.CF_Mask) != cpu.CF_Mask) {
        System.out.println( "Error: ADDrr8b: 0x10 + 0xf0 = 0x00 and CF is NOT set" );
        status = status && false;
      }
      cpu.regs[cpu.A]=0x10; cpu.regs[cpu.B]=0xf0;
      cpu.regs[cpu.FLAG_REG] = 0xf0; // set all flags
